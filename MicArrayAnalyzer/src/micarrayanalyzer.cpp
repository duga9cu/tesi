...
#define FRAMEOVERLAP 0.5					// percentage of frame overlapping
...
bool MicArrayAnalyzer::Calculate(sampleCount frame)
{
	...
	sampleCount spare=0;
	assert(startFrameSmpl>=0);
	if (startFrameSmpl < 0 ) { //it shouldn't be the case		
		RShift = -startFrameSmpl;
		startFrameSmpl += RShift;
		endFrameSmpl += RShift;
	} else	if (endFrameSmpl >= iAudioTrackLength) { //on the last frame ... cut!
		spare = endFrameSmpl - iAudioTrackLength;
	...
	for (int i=0; i<iProjectNumTracks; i++) {  //copy data into actual frame
		...
		if (endFrameSmpl == iAudioTrackLength-1) { //if last frame -> zero padding
			for (int j=endFrameSmpl-startFrameSmpl; j < endFrameSmpl - startFrameSmpl + spare; j++) {
				ActualFrameAudioData[i][j]=0;
			}
		}
	}
...
{
#ifdef __AUDEBUG__
	printf("AudioPool::GetMINResultInTheBand: band [%d]\n",col);
	fflush(stdout);
#endif
	if (bResultsMatrixAlloc)
	{
