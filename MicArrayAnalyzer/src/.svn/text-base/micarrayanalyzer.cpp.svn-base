...
bool MicArrayAnalyzer::Calculate()
   {
   #ifdef __AUDEBUG__
      printf("MicArrayAnalyzer::Calculate(): Backgroung image check and resize\n");
      fflush(stdout);
   #endif
...
//         VirtualMike* pvmDebug;
   ...
   for(i = 0; i < sfinfo.channels; i++)  //for each row of the IRs matrix (REMEMBER that sfinfo.channels and iVirtualMikes are the same thing...)
      {
         ...
         fflush(stdout);
      #endif
      afmvConvolver->SetInputVectorItem(ppfAudioData[i],i); //inside this "for" loop I load Audacity tracks too instead of using another separate "for"
   ...
   */
   ...
   for(int i=0; i<iVirtualMikes; i++)
      ...
      apOutputData->SetTrack(i,ppfAudioData[i],iAudioTrackLength,true); //***DEBUG*** BYPASSING CONVOLUTION!
      }
   DestroyProgressMeter();
...
void MicArrayAnalyzer::AudioDataInit()
   ...
   for (int i = 0; i < iProjectNumTracks; i++) ppfAudioData[i] = 0;  //Init each pointer to zero!
   }
